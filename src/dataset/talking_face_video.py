...
class TalkingFaceVideo(Dataset):
    ...
    def __getitem__(self, index):
        ...
        while flag:
            ...
            if 'info' in video_info and self.lang != 'multi':
                language = video_info["info"]["language"]
                if self.lang == 'en' and language != "English":
                    # print(f'Skip {video_path} because the language is {language}, which is not English.')
                    index += 1
                    if index >= len(self.videos_info):
                        index = 0
                    continue
